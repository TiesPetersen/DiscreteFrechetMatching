...
// Clears P's child flag for child X (mirrors the attach logic).
void clear_child_flag(std::vector<Node>& g, int P, int X, int n) {
    int xi = X/n, xj = X%n, pi = P/n, pj = P%n;
    if      (xi == pi   && xj == pj+1) g[P].child_upper    = false;
    else if (xi == pi+1 && xj == pj+1) g[P].child_diagonal = false;
    else                                g[P].child_right    = false;
}
    ...
    for (int i = 1; i < m; ++i) {
        for (int j = 1; j < n; ++j) {
            ...
            if (!has_children(g, B)) {
                ...
                // Walk the tree path from B to dpb, cleaning outgoing shortcuts
                // at each step. X's incoming lists are intentionally left as-is
                // (they are never accessed again after X dies).
                int X = B;
                while (X != dpb) {
                    int P = g[X].parent;
                    remove_from_incoming(g, g[X].out_low);
                    remove_from_incoming(g, g[X].out_high);
                    clear_child_flag(g, P, X, n);
                    X = P;
                }
                // dpb's relevant incoming list is now clean (all dead-path
                // shortcuts removed by the loop). dpb's child flag was cleared
                // in the final loop iteration.
                Shortcut* followup = extend_shortcuts(g, dpb, fdir);
