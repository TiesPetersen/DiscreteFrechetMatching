...
using ShortcutHandle = std::list<Shortcut>::iterator;
using OptionalShortcutHandle = std::optional<ShortcutHandle>;
...
struct Node {
    ...
    OptionalShortcutHandle out_low = std::nullopt;            // shortcut to the nearest ancestor in the lower direction
    OptionalShortcutHandle out_high = std::nullopt;           // shortcut to the nearest ancestor in the upper direction
    std::list<Shortcut> in_upper;        // incoming shortcuts from upper children
    std::list<Shortcut> in_diag_upper;   // incoming shortcuts from diagonal-up children
    std::list<Shortcut> in_diag_lower;   // incoming shortcuts from diagonal-down children
    std::list<Shortcut> in_right;        // incoming shortcuts from right children
};
...
ShortcutHandle get_deepest_shortcut(const std::vector<Node>& G, long long index) {
    
    OptionalShortcutHandle sc_low = G[index].out_low;
    OptionalShortcutHandle sc_high = G[index].out_high;

    if (!sc_high || (sc_low && G[(*sc_low)->target].depth > G[(*sc_high)->target].depth)) {
        return *sc_low;
    }
    else {
        return *sc_high;
...
void remove_from_incoming(std::vector<Node>& G, OptionalShortcutHandle shortcut) {
    if (!shortcut) return;
    ...
    case Direction::DOWN:       L = &G[target].in_upper;      break;
    case Direction::DIAG_UPPER: L = &G[target].in_diag_upper; break;
    case Direction::DIAG_LOWER: L = &G[target].in_diag_lower; break;
    case Direction::LEFT:       L = &G[target].in_right;      break;
    }
...
void add_shortcut(std::deque<Shortcut>& pool, std::vector<Node>& G, long long origin, bool is_low, long long target, double value, Direction direction) {
    ...
    L->emplace_back();
    ShortcutHandle sc = --L->end();
    sc->target = target;
    sc->value = value;
    sc->direction = direction;
    COUNT(shortcuts_written);

    //std::cout << "making shortcut " << origin << " -> " << target << ", low = " << is_low << std::endl;
...
void extend_shortcuts_to(std::vector<Node>& G, std::list<Shortcut>& list, ShortcutHandle followup_shortcut) {

    std::list<Shortcut>* L;
    switch (followup_shortcut->direction) {
    case Direction::DOWN:       L = &G[followup_shortcut->target].in_upper;      break;
    case Direction::DIAG_UPPER: L = &G[followup_shortcut->target].in_diag_upper; break;
    case Direction::DIAG_LOWER: L = &G[followup_shortcut->target].in_diag_lower; break;
    case Direction::LEFT:       L = &G[followup_shortcut->target].in_right;      break;
    }
    ...
    sc++;
    while (sc != L->end()) {
        sc->value = std::max(sc->value, followup_shortcut->value);
        sc->direction = followup_shortcut->direction;
        sc->target = followup_shortcut->target;
        sc++;
...
OptionalShortcutHandle extend_shortcuts(std::vector<Node>& G, long long dead_path_base, Direction final_direction) {
...
void adjust_ncas(SelectResult& select_results, long long dead_path_base, OptionalShortcutHandle followup_shortcut) {
    if (!followup_shortcut) return;
    ShortcutHandle followup = *followup_shortcut;
    if (select_results.nca_AB == dead_path_base) {
        select_results.nca_AB   = followup->target;
        select_results.max_A_AB = std::max(select_results.max_A_AB, followup->value);
        select_results.max_B_AB = std::max(select_results.max_B_AB, followup->value);
        select_results.dir_A_AB = followup->direction;
        select_results.dir_B_AB = followup->direction;
    }
    if (select_results.nca_BC == dead_path_base) {
        select_results.nca_BC   = followup->target;
        select_results.max_B_BC = std::max(select_results.max_B_BC, followup->value);
        select_results.max_C_BC = std::max(select_results.max_C_BC, followup->value);
        select_results.dir_B_BC = followup->direction;
        select_results.dir_C_BC = followup->direction;
    }
    if (select_results.nca_AC == dead_path_base) {
        select_results.nca_AC   = followup->target;
        select_results.max_A_AC = std::max(select_results.max_A_AC, followup->value);
        select_results.max_C_AC = std::max(select_results.max_C_AC, followup->value);
        select_results.dir_A_AC = followup->direction;
        select_results.dir_C_AC = followup->direction;
    }
}
