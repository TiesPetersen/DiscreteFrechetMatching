...
- `datasets/` -> generated curve-pair files consumed by the experiment, split into `synthetic/` and `real-world/` (mirroring `experiment/`'s split below):
    ...
    - `real-world/`: curve pairs sampled from public trajectory datasets (Geolife, Pigeons, Drifter), generated by `generate_real_world_datasets.py` from raw downloads expected under `external_datasets/` (gitignored). Unlike the synthetic sets, curves here have independent, unequal lengths and no shared N grid, and each dataset's pairs are split across multiple `part_NNN.txt` files to stay under GitHub's 100MB per-file limit.
