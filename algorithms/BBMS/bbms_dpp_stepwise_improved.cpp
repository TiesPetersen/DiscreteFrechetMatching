...
#include "bbms_dpp_common_improved.h"
...
MatchingAndFrechetDistance bbms_dpp_stepwise_improved(const Curve& p, const Curve& q) {
    ...
    for (long long i = 1; i < m; ++i) {
        for (long long j = 1; j < n; ++j) {
            ...
            long long parent = select_parent(G, A, B, C, n);
            attach(G, parent, D, n);
            TRACE_PARENT(parent);
            ...
            if (!has_children(G, B)) {
                ...
                Shortcut* followup = extend_shortcuts(G, dead_path_base, final_direction);
            ...
            update_shortcuts(pool, G, A, B, C, D, parent);
        }
    }
