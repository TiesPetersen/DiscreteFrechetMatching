...
enum class Direction { DOWN, DIAG_UP, DIAG_LOW, LEFT };
...
void remove_from_incoming(std::vector<Node>& G, Shortcut* shortcut) {
    ...
    switch (shortcut->direction) {
        ...
        case Direction::DIAG_UP:  remove_from(G[target].in_diag_upper, shortcut); break;
        case Direction::DIAG_LOW: remove_from(G[target].in_diag_lower, shortcut); break;
...
void add_to_incoming(std::vector<Node>& G, Shortcut* shortcut) {
    ...
    switch (shortcut->direction) {
        ...
        case Direction::DIAG_UP:  G[target].in_diag_upper.push_back(shortcut); break;
        case Direction::DIAG_LOW: G[target].in_diag_lower.push_back(shortcut); break;
...
Shortcut* extend_shortcuts(std::vector<Node>& G, long long dead_path_base, Direction final_direction) {
    ...
    } else if (final_direction == Direction::DIAG_UP || final_direction == Direction::DIAG_LOW) {
...
void update_shortcuts(std::deque<Shortcut>& pool, std::vector<Node>& G, long long A, long long B, long long C, long long D, const SelectResult& select_results) {
    ...
    if (select_results.parent == A) {
        ...
        if (AB && BC) {
            add_shortcut(pool, G, A, true, B, G[A].distance, Direction::DOWN);
            add_shortcut(pool, G, C, false, B, G[C].distance, Direction::LEFT);
            add_shortcut(pool, G, D, false, B, G[D].distance, Direction::DIAG_UP);
            add_shortcut(pool, G, D, true, B, G[D].distance, Direction::DIAG_LOW);
        } else if (AB) {
            add_shortcut(pool, G, A, true, B, G[A].distance, Direction::DOWN);
            add_shortcut(pool, G, D, false, B, G[D].distance, Direction::DIAG_UP);
