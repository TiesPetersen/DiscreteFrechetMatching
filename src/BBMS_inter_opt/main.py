...
def BBMS_inter_opt(curve1, curve2):
    ...
    while u != v:
        if v.depth > u.depth:
            ...
            if v_high is not None:
                v_high_value = v_high.value
                if v_high_value > max_distance_v:
                    max_distance_v = v_high_value
                v = v_high.target
            else:
                v_dist = v.distance
                if v_dist > max_distance_v:
                    max_distance_v = v_dist
                v = v.parent
        else:
            # u should take low shortcut if it exists
            u_low = u.low
            if u_low is not None:
                u_low_value = u_low.value
                if u_low_value > max_distance_u:
                    max_distance_u = u_low_value
                u = u_low.target
            else:
                u_dist = u.distance
                if u_dist > max_distance_u:
                    max_distance_u = u_dist
                u = u.parent
...
def updateShortcuts(G, i, j, extra_info, A, B, C, D):
    ...
    D_dist = D.distance
    max_A_AB = extra_info['max_A_AB']
    max_B_AB = extra_info['max_B_AB']
    nca_AB   = extra_info['nca_AB']
    max_B_BC = extra_info['max_B_BC']
    max_C_BC = extra_info['max_C_BC']
    nca_BC   = extra_info['nca_BC']
    max_A_AC = extra_info['max_A_AC']
    max_C_AC = extra_info['max_C_AC']
    nca_AC   = extra_info['nca_AC']
    ...
    if extra_info['selectedParent'] == A:
        ...
        if AB and BC:
            ...
            D.low = Shortcut(B, A.distance if A.distance > D_dist else D_dist)
        elif AB:
            A.low = Shortcut(nca_AC, max_A_AC)
            D.low = Shortcut(nca_AC, max_A_AC if max_A_AC > D_dist else D_dist)
        elif BC:
            C.high = Shortcut(nca_AC, max_C_AC)
            D.low = Shortcut(nca_AC, max_A_AC if max_A_AC > D_dist else D_dist)
        else:
            D.low = Shortcut(nca_AB, max_A_AB if max_A_AB > D_dist else D_dist)
    elif extra_info['selectedParent'] == B:
        # chosen parent is B
        if AB and BC:
            A.low = Shortcut(B, A.distance)
            C.high = Shortcut(B, C.distance)
            D.high = Shortcut(B, D_dist)
            D.low = Shortcut(B, D_dist)
        elif AB:
            A.low = Shortcut(B, A.distance)
            D.high = Shortcut(B, D_dist)
            D.low = Shortcut(nca_AC, max_B_BC if max_B_BC > D_dist else D_dist)
        elif BC:
            C.high = Shortcut(B, C.distance)
            D.high = Shortcut(nca_AC, max_B_AB if max_B_AB > D_dist else D_dist)
            D.low = Shortcut(B, D_dist)
        else:
            D.high = Shortcut(nca_AB, max_B_AB if max_B_AB > D_dist else D_dist)
            D.low = Shortcut(nca_BC, max_B_BC if max_B_BC > D_dist else D_dist)
    elif extra_info['selectedParent'] == C:
        # chosen parent is C
        if AB and BC:
            A.low = Shortcut(B, A.distance)
            C.high = Shortcut(B, C.distance)
            D.high = Shortcut(B, C.distance if C.distance > D_dist else D_dist)
        elif AB:
            A.low = Shortcut(nca_AC, max_A_AC)
            D.high = Shortcut(nca_AC, max_C_AC if max_C_AC > D_dist else D_dist)
        elif BC:
            C.high = Shortcut(nca_AC, max_C_AC)
            D.high = Shortcut(nca_AC, max_C_AC if max_C_AC > D_dist else D_dist)
        else:
            D.high = Shortcut(nca_BC, max_C_BC if max_C_BC > D_dist else D_dist)
