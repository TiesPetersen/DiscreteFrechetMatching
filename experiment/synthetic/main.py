...
Run from the project root:
    python3 experiment/synthetic/main.py
"""
...
DATASETS_DIR = os.path.join("datasets", "synthetic")
RESULTS_DIR  = "results"
LOG_PATH     = os.path.join(RESULTS_DIR, "experiment.log")

REPEAT     = 3     # timing repeats per sample
...
def log(msg):
    """Prints `msg` and appends it to LOG_PATH, both timestamped in Amsterdam local time."""
...
def read_dataset_header(path):
    """Reads just enough of a dataset file to get (K, first_sample_m) without loading the whole file"""
    with open(path) as f:
        head = f.read(256)
    tokens = head.split()
    if len(tokens) < 2:
...
def discover_datasets():
    ...
    Directories with no N_*.txt files are excluded.
    ...
    dataset, and each file's filename-claimed N must match the actual point count
    of its first curve."""

    # Check for the datasets/ folder
    if not os.path.isdir(DATASETS_DIR):
        log(f"ERROR: no '{DATASETS_DIR}/' folder found.")
        return None

    # Scan every dataset folder and collect its grid and K values
    datasets = {}
    for name in sorted(os.listdir(DATASETS_DIR)):
        ...
        # Scan every N_*.txt file in this dataset folder and collect its N and K values
        grid = []
        k_by_n = {}
        ...
        # Exclude datasets with no N_*.txt files
        if not grid:
            continue
    ...
    # Report an error if no dataset folders were found at all
    if not datasets:
        log(f"ERROR: no dataset folders with N_*.txt files found under '{DATASETS_DIR}/'.")
        return None

    # Check that every dataset has the same grid
...
def main():
    ...
    # Create results/ and open the log file for appending
    os.makedirs(RESULTS_DIR, exist_ok=True)
    _log_file = open(LOG_PATH, "a")

    log("Starting run.")

    # Check that the runner binaries exist
    ...
    # Discover datasets and their grids/K values
    datasets = discover_datasets()
    if datasets is None:
        sys.exit(1)

    # Report the discovered datasets and their grids/K values
    ...
    # Create results/NAME/ for every dataset, and load any existing results from previous runs
    for name in datasets:
        os.makedirs(os.path.join(RESULTS_DIR, name), exist_ok=True)

    # Load existing results and reconstruct the done_timing, done_opcount, walls, and opcount_rows structures
    ...
    # Run every (dataset, N, sample) combination for both passes, skipping any that are already done or walled
    for dataset in sorted(datasets):
        ...
        # Run every (N, sample) combination for this dataset
        for n in grid:
            log(f"--- N={n} ---")
            dataset_file = os.path.join(DATASETS_DIR, dataset, f"N_{n}.txt")
            k = k_by_n[n]

            # Run every sample for this (dataset, N) combination
            for sample in range(k):
                log(f"sample={sample}")

                # Run the timing/memory pass REPEAT times for this (dataset, N, sample) combination
                log("runtime/memory pass")
                for repeat in range(REPEAT):
                    # Run every algorithm for this (dataset, N, sample, repeat) combination
                    for algorithm in ALGORITHMS:
                        # Skip any algorithm that has already failed on a smaller N for this dataset
                        wall = walls.get((dataset, algorithm))
                        if wall is not None and n >= wall:
                            log(f"  repeat={repeat} {algorithm}: SKIP (walled at N={wall})")
                            continue

                        # Skip any algorithm that has already completed this exact (dataset, N, sample, repeat) combination
                        ...
                        # Run the algorithm and classify the result
                        ...
                        # If this run failed, update the wall for this (dataset, algorithm) pair to the smallest N that has failed so far
                ...
                # Run the op-count pass once for this (dataset, N, sample) combination
                log("op-count pass")
                for algorithm in ALGORITHMS:
                    # Skip any algorithm that has already failed on a smaller N for this dataset
                    wall = walls.get((dataset, algorithm))
                    ...
                    # Skip any algorithm that has already completed this exact (dataset, N, sample) combination
                ...
                # Check that every algorithm which reported status=ok on this (dataset, N, sample) agrees on frechet_distance
                ok = check_cross_agreement(dataset, n, sample, opcount_rows)
                if ok:
                    log("cross-algorithm agreement: ok")
                else:
                    log("cross-algorithm agreement: FAIL")
